...
[Dorado](https://github.com/nanoporetech/dorado) は Oxford Nanopore 社の DNA シーケンサーのために開発されたベースコーラーである。ベースコーラーとは、シーケンサーがDNA分子を測定して得られた波形データを塩基（ベース）配列に変換するソフトウェアである。波形の変換には機械学習モデルが用いられており、数TBの波形データを高速にかつ正確に塩基配列に変換するためのアクセラレータとしてGPUが用いられている．
...
| **p4d.24xlarge (A100x8)** | v0.2.4      | 48.00                                  | 48.00                          | 54.00                               |
| **p3.8xlarge (V100x4)**   | v0.2.4      | 258.00                                 | 264.00                         | 282.00                              |
